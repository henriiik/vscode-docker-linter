...
import {
    ...
    TextDocuments, TextDocument, TextDocumentSyncKind,
    ErrorMessageTracker, Range
...
function getDiagnostic(match: RegExpMatchArray): Diagnostic {
    ...
    let range = Range.create(line, start, line, end);
    let diagnostic = Diagnostic.create(range, match[settings.message], severity, match[settings.code], settings.linter);
...
function validate(document: TextDocument): void {
    ...
    connection.console.log(`Wants to validate ${uri} ${Date.now()}`);
    ...
    child.stdin.on("data", (data: any) => {
        connection.console.log('datain!' + data);
    });

    child.stderr.on("data", (data: any) => {
        connection.console.log('dataerror!' + data);
        debugString += data;
        diagnostics = diagnostics.concat(parseBuffer(data));
    });

    child.stdout.on("data", (data: any) => {
        connection.console.log("dataout! " + data);
        debugString += data;
        diagnostics = diagnostics.concat(parseBuffer(data));
    });

    child.on("error", (some: any) => {
        connection.console.log('someerror!'+some);
    });

    child.on('exit', () => {
        connection.console.log('exit!');
    })
    ...
    child.on("close", (code: string) => {
        if (debugString.match(/^Error response from daemon/)) {
            ...
            connection.console.log("debug! "+debugString);
